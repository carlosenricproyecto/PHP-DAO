<?php
/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */
include("header.php");
?>
<div class="container-fluid">
    <div class="row">
        <div class="col-md-1"></div>
        <div class="col-md-5">
            <a href="../../controller/newMechTeam.php" class="thumbnail menuthumb">
                <span class="glyphicon glyphicon-plus menuicon"></span>
                <p class="menutext">Add Mech Team</p>
            </a>
        </div>
        <div class="col-md-5">
            <div class="thumbnail menuthumb">
                <span class="glyphicon glyphicon-edit menuicon"></span>
                <p class="menutext">Edit Mech Team</p>
            </div>
        </div>
        <div class="col-md-1"></div>
    </div>
    <div class="row">
        <div class="col-md-1"></div>
        <div class="col-md-5">
            <a href="../../controller/generateList.php?object=Mechanic Teams" class="thumbnail menuthumb">
                <span class="glyphicon glyphicon-list-alt menuicon"></span>
                <p class="menutext">List Mech Teams</p>
            </a>
        </div>
        <div class="col-md-5">
            <div class="thumbnail menuthumb">
                <span class="glyphicon glyphicon-remove menuicon"></span>
                <p class="menutext">Delete Mech Teame</p>
            </div>
        </div>
        <div class="col-md-1"></div>
        </div>
    </div>

